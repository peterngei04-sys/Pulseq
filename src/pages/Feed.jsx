...
const handleShare = async (post, e) => {
  ...
  const message = `🔥 This is heating up on PulseQ!\n\n"${post.title}"\n\nJoin the conversation — real people, real thoughts, no filters. 👇\n\n${link}\n\nDownload PulseQ & be part of the pulse. 🚀`;

  try {
    if (navigator.share) {
      await navigator.share({
        title: post.title,
        text: `🔥 "${post.title}" is trending on PulseQ — join the conversation! 🚀`,
        url: link
      });
    } else {
...
const PostBody = ({ body }) => {
  ...
  const LIMIT = 180;
  const isLong = body && body.length > LIMIT;

  return (
    <p className="modern-body">
      {isLong && !expanded ? body.slice(0, LIMIT) + "..." : body}
      {isLong && (
        <span
